...
const ReportsPage = () => {
  ...
  // Re-download event PDF from history
  const handleRedownloadEventPDF = (eventId: string) => {
    const event = events.find(e => e.id === eventId);
    if (!event) return;

    const evtCotisations = cotisations.filter(c => c.eventId === eventId);
    const evtTransactions = transactions.filter(t => t.eventId === eventId);
    ...
    doc.setFontSize(14);
    doc.text(`Rapport Financier - ${event.name}`, pageWidth / 2, 32, { align: 'center' });
    ...
    doc.text(`Date de l'événement: ${eventDate}`, 14, 55);
    doc.text(`Date du rapport: ${new Date().toLocaleDateString('fr-FR')}`, 14, 62);
    doc.text(`Cotisation Homme: ${Math.floor(event.cotisationHomme)} F CFA`, 14, 69);
    doc.text(`Cotisation Femme: ${Math.floor(event.cotisationFemme)} F CFA`, 14, 76);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Total des entrées', `${Math.floor(cotTotal + incTotal)} F`],
        ['Total des dépenses', `${Math.floor(expTotal)} F`],
        ['Solde final', `${Math.floor(cotTotal + incTotal - expTotal)} F`],
      ],
      theme: 'striped',
      headStyles: { fillColor: [14, 165, 233] },
      styles: { fontSize: 11 },
    ...
    const fileName = `rapport-${event.name.replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };

  // Re-download annual PDF from history
  const handleRedownloadAnnualPDF = (year: string) => {
    const yearEvents = events.filter(e => new Date(e.date).getFullYear() === parseInt(year));
    ...
    doc.setFontSize(14);
    doc.text(`Rapport Annuel ${year}`, pageWidth / 2, 32, { align: 'center' });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setTextColor(128, 128, 128);
      doc.text(
        `Page ${i} sur ${pageCount} - Rapport Annuel ${year} - Généré le ${new Date().toLocaleDateString('fr-FR')}`,
        pageWidth / 2,
        doc.internal.pageSize.getHeight() - 10,
        { align: 'center' }
      );
          ...
          <CardContent>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {reportHistory.slice(0, 20).map((report) => {
                const canDownload = report.type === 'annual' 
                  ? !!report.year 
                  : !!report.eventId && events.some(e => e.id === report.eventId);
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {canDownload && (
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            if (report.type === 'annual' && report.year) {
                              handleRedownloadAnnualPDF(report.year);
                            } else if (report.eventId) {
                              handleRedownloadEventPDF(report.eventId);
                            }
                          }}
                        >
